...
export const NULLABLE_TYPES_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See a nullable type hold both a real value and null.', cards: [
    { id: 'nullabletypes-explore-1', number: '01', title: 'A nullable String holding text', language: 'Kotlin', subtitle: 'String? can hold ordinary text, just like String.', code: ['val city: String? = "Pune"', 'println(city)'], whatItMeans: [{ label: 'String?', description: 'A nullable type can still hold a normal, real value.' }], whatChanged: 'Declared a nullable type and gave it a real value.' },
    { id: 'nullabletypes-explore-2', number: '02', title: 'The same variable holding null', language: 'Kotlin', subtitle: 'Only a nullable type can be assigned null.', code: ['val city: String? = null', 'println(city)'], whatItMeans: [{ label: 'null', description: 'Represents "no value at all" — printed as the literal text null.' }], whatChanged: 'Used null to represent a genuinely missing value.' },
    { id: 'nullabletypes-explore-3', number: '03', title: 'A nullable Int', language: 'Kotlin', subtitle: 'Nullability applies to any type, not just String.', code: ['val bonus: Int? = null', 'println(bonus)'], whatItMeans: [{ label: 'Int?', description: 'An Int that may or may not have a value — here, it has none.' }], whatChanged: 'Applied nullability to a numeric type.' },
  ] },
...
export const NULLABLE_VARIABLES_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'Reassign a nullable var in both directions.', cards: [
    { id: 'nullablevars-explore-1', number: '01', title: 'From null to a real value', language: 'Kotlin', subtitle: 'A nullable var can gain a value after starting as null.', code: ['var status: String? = null', 'status = "Active"', 'println(status)'], whatItMeans: [{ label: 'status = "Active"', description: 'Reassigns status from null to real text.' }], whatChanged: 'Moved a nullable var from missing to present.' },
    { id: 'nullablevars-explore-2', number: '02', title: 'From a real value to null', language: 'Kotlin', subtitle: 'A nullable var can lose its value just as easily.', code: ['var token: String? = "abc123"', 'token = null', 'println(token)'], whatItMeans: [{ label: 'token = null', description: 'Reassigns token from real text back to null.' }], whatChanged: 'Moved a nullable var from present to missing.' },
    { id: 'nullablevars-explore-3', number: '03', title: 'Only the latest assignment counts', language: 'Kotlin', subtitle: 'Multiple reassignments still leave only the final value.', code: ['var session: String? = null', 'session = "started"', 'session = null', 'println(session)'], whatItMeans: [{ label: 'session = null', description: 'The final reassignment wins, no matter what came before it.' }], whatChanged: 'Confirmed that only the most recent assignment survives.' },
  ] },
...
export const SAFE_CALL_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See ?. behave differently depending on whether the value is null.', cards: [
    { id: 'safecall-explore-1', number: '01', title: 'Safe call on a real value', language: 'Kotlin', subtitle: '?. works exactly like . when there’s no null.', code: ['val name: String? = "Kotlin"', 'println(name?.length)'], whatItMeans: [{ label: 'name?.length', description: 'name is not null, so length is read normally: 6.' }], whatChanged: 'Read a property safely from a nullable value that happened to be present.' },
    { id: 'safecall-explore-2', number: '02', title: 'Safe call on null', language: 'Kotlin', subtitle: '?. skips the access instead of crashing.', code: ['val name: String? = null', 'println(name?.length)'], whatItMeans: [{ label: 'name?.length', description: 'name is null, so length is never accessed — the whole expression becomes null.' }], whatChanged: 'Avoided a crash by letting the safe call produce null instead.' },
    { id: 'safecall-explore-3', number: '03', title: 'The program keeps running', language: 'Kotlin', subtitle: 'A safe call that hits null doesn’t stop the program.', code: ['val name: String? = null', 'println(name?.length)', 'println("Program kept running")'], whatItMeans: [{ label: 'println("Program kept running")', description: 'This line still executes, proving the safe call didn’t crash anything above it.' }], whatChanged: 'Confirmed the program continues normally after a safe call hits null.' },
  ] },
...
export const ELVIS_OPERATOR_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'Supply fallbacks for missing values, including alongside a safe call.', cards: [
    { id: 'elvis-explore-1', number: '01', title: 'A fallback when null', language: 'Kotlin', subtitle: '?: kicks in only when the left side is null.', code: ['val nickname: String? = null', 'println(nickname ?: "Guest")'], whatItMeans: [{ label: 'nickname ?: "Guest"', description: 'nickname is null, so the fallback "Guest" is used.' }], whatChanged: 'Replaced a missing value with a sensible default.' },
    { id: 'elvis-explore-2', number: '02', title: 'The real value wins when present', language: 'Kotlin', subtitle: '?: has no effect when the left side already has a value.', code: ['val nickname: String? = "Zed"', 'println(nickname ?: "Guest")'], whatItMeans: [{ label: 'nickname ?: "Guest"', description: 'nickname is not null, so "Guest" is never used.' }], whatChanged: 'Confirmed the fallback is skipped when a real value exists.' },
    { id: 'elvis-explore-3', number: '03', title: 'Combining ?. and ?:', language: 'Kotlin', subtitle: 'A safe call and a fallback often appear together.', code: ['val name: String? = null', 'println(name?.length ?: 0)'], whatItMeans: [{ label: 'name?.length ?: 0', description: 'name is null, so ?. produces null, and ?: replaces that null with 0.' }], whatChanged: 'Chained a safe call into a fallback for a clean final value.' },
  ] },
...
export const NON_NULL_ASSERTION_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See !! succeed when you’re right, and crash when you’re wrong.', cards: [
    { id: 'nonnull-explore-1', number: '01', title: '!! succeeds on a real value', language: 'Kotlin', subtitle: 'When the assertion is correct, !! just returns the value.', code: ['val name: String? = "Sam"', 'println(name!!.length)'], whatItMeans: [{ label: 'name!!', description: 'Asserts name is non-null, which is true here, so .length reads normally.' }], whatChanged: 'Used !! successfully because the value truly wasn’t null.' },
    { id: 'nonnull-explore-2', number: '02', title: '!! crashes on null', language: 'Kotlin', subtitle: 'When the assertion is wrong, the program stops immediately.', code: ['val name: String? = null', 'println(name!!.length)'], whatItMeans: [{ label: 'name!!', description: 'Asserts name is non-null, but name really is null, so this throws instead of returning anything.' }], whatChanged: 'Demonstrated exactly the crash !! risks when the assertion is false.' },
    { id: 'nonnull-explore-3', number: '03', title: '!! used alone', language: 'Kotlin', subtitle: '!! can also just be used to read the value itself.', code: ['val age: Int? = 25', 'println(age!!)'], whatItMeans: [{ label: 'age!!', description: 'Returns age’s value directly, now treated as a plain, non-null Int.' }], whatChanged: 'Asserted non-nullness without also accessing a property.' },
  ] },
...
export const NULL_CHECKS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'Branch based on whether a nullable value is present or missing.', cards: [
    { id: 'nullchecks-explore-1', number: '01', title: 'A value is present', language: 'Kotlin', subtitle: 'if (x != null) runs its branch when x truly has a value.', code: ['val email: String? = "a@b.com"', 'if (email != null) {', '    println("Email: " + email)', '} else {', '    println("No email")', '}'], whatItMeans: [{ label: 'email != null', description: 'True, since email holds a real address, so the first branch runs.' }], whatChanged: 'Confirmed a nullable value was present before using it.' },
    { id: 'nullchecks-explore-2', number: '02', title: 'A value is missing', language: 'Kotlin', subtitle: 'The same check correctly falls into else when null.', code: ['val email: String? = null', 'if (email != null) {', '    println("Email: " + email)', '} else {', '    println("No email")', '}'], whatItMeans: [{ label: 'email != null', description: 'False, since email is null, so the else branch runs instead.' }], whatChanged: 'Avoided using a missing value by branching around it.' },
    { id: 'nullchecks-explore-3', number: '03', title: 'Checking the reverse way', language: 'Kotlin', subtitle: '== null is just as valid when that’s the case worth handling first.', code: ['val phone: String? = null', 'if (phone == null) {', '    println("Please add a phone number")', '}'], whatItMeans: [{ label: 'phone == null', description: 'True here, so the reminder message prints.' }], whatChanged: 'Checked directly for the missing case instead of the present one.' },
  ] },
...
export const SMART_CASTS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See a null check unlock plain . access inside its own block.', cards: [
    { id: 'smartcasts-explore-1', number: '01', title: 'Direct access after confirming non-null', language: 'Kotlin', subtitle: 'No ?. is needed once the check has already confirmed a value.', code: ['val name: String? = "Kotlin"', 'if (name != null) {', '    println(name.length)', '}'], whatItMeans: [{ label: 'name.length', description: 'Plain . works here because name != null already guaranteed a real value.' }], whatChanged: 'Used ordinary property access safely, thanks to the earlier check.' },
    { id: 'smartcasts-explore-2', number: '02', title: 'The else branch still handles null', language: 'Kotlin', subtitle: 'Smart casting doesn’t remove the need for an else case.', code: ['val name: String? = null', 'if (name != null) {', '    println(name.length)', '} else {', '    println("No name to measure")', '}'], whatItMeans: [{ label: 'else', description: 'Handles the case where name really is null, since the if branch never runs.' }], whatChanged: 'Combined a smart-cast branch with a fallback for the missing case.' },
    { id: 'smartcasts-explore-3', number: '03', title: 'A parameter may still be null after the branch', language: 'Kotlin', subtitle: 'If both paths continue, the outside access must handle null.', code: ['fun report(city: String?) {', '    if (city != null) {', '        println(city.length)', '    }', '    println(city?.length)', '}', 'fun main() {', '    report(null)', '}'], whatItMeans: [{ label: 'city.length', description: 'Plain . works inside the if block, where city is smart-cast to non-null.' }, { label: 'city?.length', description: 'The null path also reaches this line, so this access uses ?.. This is about reaching paths, not simply closing a brace.' }], whatChanged: 'Distinguished a branch-local proof from a function-wide early-return guard.' },
  ] },
...
export const SAFE_CASTS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See a safe cast succeed, fail, and combine with a fallback.', cards: [
    { id: 'safecasts-explore-1', number: '01', title: 'A cast that succeeds', language: 'Kotlin', subtitle: 'The value genuinely matches the target type.', code: ['val obj: Any = "hello"', 'val result = obj as? String', 'println(result)'], whatItMeans: [{ label: 'obj as? String', description: 'obj really is a String, so the cast succeeds and returns it.' }], whatChanged: 'Cast a general value into its specific type safely.' },
    { id: 'safecasts-explore-2', number: '02', title: 'A cast that fails', language: 'Kotlin', subtitle: 'A mismatched type produces null instead of crashing.', code: ['val obj: Any = 42', 'val result = obj as? String', 'println(result)'], whatItMeans: [{ label: 'obj as? String', description: 'obj is actually an Int, not a String, so the cast fails and produces null.' }], whatChanged: 'Confirmed a failed safe cast produces null rather than an error.' },
    { id: 'safecasts-explore-3', number: '03', title: 'Combining as? with a fallback', language: 'Kotlin', subtitle: 'as? and ?: pair naturally together.', code: ['val obj: Any = 42', 'val text = obj as? String ?: "not text"', 'println(text)'], whatItMeans: [{ label: 'obj as? String ?: "not text"', description: 'The cast fails (obj isn’t a String), so the ?: fallback supplies "not text".' }], whatChanged: 'Turned a failed cast into a clean, usable default value.' },
  ] },
...
export const NULLABLE_COLLECTIONS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See nullable elements, a nullable collection reference, and a Map with nullable values.', cards: [
    { id: 'nullablecollections-explore-1', number: '01', title: 'A list with nullable elements', language: 'Kotlin', subtitle: 'The list is real; some entries inside are null.', code: ['val ages: List<Int?> = listOf(20, null, 25)', 'println(ages)'], whatItMeans: [{ label: 'List<Int?>', description: 'The list itself always exists; individual ages may be null.' }], whatChanged: 'Stored a mix of real values and missing ones in one list.' },
    { id: 'nullablecollections-explore-2', number: '02', title: 'A nullable list reference', language: 'Kotlin', subtitle: 'The list itself might not exist at all.', code: ['val names: List<String>? = null', 'println(names?.size)'], whatItMeans: [{ label: 'names?.size', description: 'names is null, so a safe call is required — the whole expression becomes null.' }], whatChanged: 'Safely handled a collection reference that might not exist yet.' },
    { id: 'nullablecollections-explore-3', number: '03', title: 'A Map with nullable values', language: 'Kotlin', subtitle: 'Some entries can map to null instead of a real value.', code: ['val scores: Map<String, Int?> = mapOf("Tom" to null, "Ana" to 90)', 'println(scores["Tom"])', 'println(scores["Ana"])'], whatItMeans: [{ label: 'scores["Tom"]', description: 'Tom’s score is null — recorded, but not yet given a real value.' }], whatChanged: 'Modeled a lookup table where some values are deliberately missing.' },
  ] },
...
export const CHAINING_NULLABLE_OPERATIONS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'See the same chained expression handle three different kinds of missing data.', cards: [
    { id: 'chaining-explore-1', number: '01', title: 'The chain reaches a real value', language: 'Kotlin', subtitle: 'When nothing is missing, the chain just returns the real result.', code: ['val nicknames: Map<String, String?> = mapOf("Tom" to null, "Ana" to "Az")', 'println(nicknames["Ana"]?.length ?: 0)'], whatItMeans: [{ label: 'nicknames["Ana"]?.length', description: 'Ana’s nickname exists, so .length reads normally: 2.' }], whatChanged: 'Confirmed the chain works exactly like a single safe call when nothing is missing.' },
    { id: 'chaining-explore-2', number: '02', title: 'A null value inside the chain', language: 'Kotlin', subtitle: 'The key exists, but its value is null.', code: ['val nicknames: Map<String, String?> = mapOf("Tom" to null, "Ana" to "Az")', 'println(nicknames["Tom"]?.length ?: 0)'], whatItMeans: [{ label: 'nicknames["Tom"]?.length', description: 'Tom’s nickname is null, so ?. short-circuits, and ?: 0 supplies the fallback.' }], whatChanged: 'Handled a present key with a missing value using the same chain.' },
    { id: 'chaining-explore-3', number: '03', title: 'A missing key entirely', language: 'Kotlin', subtitle: 'The key was never even added.', code: ['val nicknames: Map<String, String?> = mapOf("Ana" to "Az")', 'println(nicknames["Zed"]?.length ?: 0)'], whatItMeans: [{ label: 'nicknames["Zed"]', description: '"Zed" was never a key at all, so the lookup itself produces null, and the same chain still handles it cleanly.' }], whatChanged: 'Confirmed the exact same chain also covers a completely missing key.' },
  ] },
...
export const WORLD_7_BOSS_LESSON: FiveStageLesson = {
  ...
  explore: { title: 'Explore the Concept', subtitle: 'Report on a dataset with missing scores, count what’s actually present, and supply a safe default for one lookup.', cards: [
    { id: 'boss7-explore-1', number: '01', title: 'Reporting every entry safely', language: 'Kotlin', subtitle: 'Each entry is handled whether its score is present or missing.', code: ['val scores: Map<String, Int?> = mapOf("Ann" to 82, "Ben" to null, "Cy" to 91)', 'for ((name, score) in scores) {', '    if (score != null) {', '        println(name + ": " + score)', '    } else {', '        println(name + ": no score yet")', '    }', '}'], whatItMeans: [{ label: 'if (score != null)', description: 'Smart-casts score for a real report line; the else branch handles the missing case cleanly.' }], whatChanged: 'Reported on every student, including the one with no score yet.' },
    { id: 'boss7-explore-2', number: '02', title: 'Counting only the real scores', language: 'Kotlin', subtitle: 'An accumulator combined with a null check counts only valid entries.', code: ['val scores: Map<String, Int?> = mapOf("Ann" to 82, "Ben" to null, "Cy" to 91)', 'var validCount = 0', 'for ((name, score) in scores) {', '    if (score != null) {', '        validCount += 1', '    }', '}', 'println(validCount)'], whatItMeans: [{ label: 'validCount += 1', description: 'Only increments when score truly isn’t null, so validCount ends at 2, not 3.' }], whatChanged: 'Summarized how much real data the dataset actually contains.' },
    { id: 'boss7-explore-3', number: '03', title: 'A safe default for one lookup', language: 'Kotlin', subtitle: '?: supplies a clean value when a lookup produces null.', code: ['val scores: Map<String, Int?> = mapOf("Ben" to null)', 'val score = scores["Ben"] ?: 0', 'println(score)'], whatItMeans: [{ label: 'scores["Ben"] ?: 0', description: 'Ben’s score is null, so the fallback 0 is used instead.' }], whatChanged: 'Turned a missing value into a safe, usable default in one line.' },
  ] },
