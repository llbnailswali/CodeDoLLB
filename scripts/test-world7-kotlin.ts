...
const dir = mkdtempSync(join(tmpdir(), 'codedo-world7-kotlin-'));
...
const previousExport = process.env.WORLD7_REFERENCE_EXPORT;
try {
 const exported = join(dir, 'cases.json');
 process.env.WORLD7_REFERENCE_EXPORT = exported;
 await import('./audit-world7-quality');
 ...
 console.log(`Kotlin World 7 reference passed: ${valid.length} runnable cases, ${invalid.length} compiler rejections.`);
} finally {
 if (previousExport === undefined) delete process.env.WORLD7_REFERENCE_EXPORT;
 else process.env.WORLD7_REFERENCE_EXPORT = previousExport;
